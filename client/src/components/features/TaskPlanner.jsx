...
const TaskPlanner = () => {
        ...
        position: {
          x: Math.max(
            ...
            Math.min(
              ...
              Math.random() * (window.innerWidth - 320)
            )
          ),
          y: Math.max(
            padding + 80,
            Math.min(
              ...
              Math.random() * (window.innerHeight - 220)
            )
          ),
        },
        dueDate: "",
  ...
  const handleDeleteTask = (taskId) => {
    ...
    setConnections(
      connections.filter((conn) => conn.start !== taskId && conn.end !== taskId)
    );
  };
  ...
  const handleDrag = (e, taskId) => {
    if (draggedTask === taskId && e.clientX && e.clientY) {
      const updatedTasks = tasks.map((task) => {
        if (task.id === taskId) {
          const newX = Math.max(
            0,
            Math.min(window.innerWidth - 300, e.clientX - 150)
          );
          const newY = Math.max(
            0,
            Math.min(window.innerHeight - 100, e.clientY - 30)
          );
  ...
  const handleTaskClick = (taskId) => {
    if (connectingMode) {
      if (!connectingStart) {
        ...
        setConnections([
          ...connections,
          { start: connectingStart, end: taskId },
        ]);
